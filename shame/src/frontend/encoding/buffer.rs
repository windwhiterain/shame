    ...
    AS: BufferAddressSpace,
{
    ...
    fn new(args: Result<BindingArgs, InvalidReason>) -> Self {
        if T::IS_ZST {
            // ZST: no IR, no layout check, no binding
            let _ = args;
            return Self { inner: BufferInner::Zst(PhantomData) };
        }
    ...
    AM: AccessModeReadable,
{
    ...
    fn new(args: Result<BindingArgs, InvalidReason>) -> Self {
        if T::IS_ZST {
            let _ = args;
            return Self { inner: BufferRefInner::Zst(PhantomData) };
        }
        let skip_stride_check = true; // not a vertex buffer
        Context::try_with(call_info!(), |ctx| {
            get_layout_compare_with_cpu_push_error::<T>(ctx, skip_stride_check)
...
pub enum BufferInner<T: GpuStore, AS: BufferAddressSpace> {
    ...
    /// ZST types have no GPU representation
    Zst(PhantomData<T>),
}
    ...
    pub(crate) fn new_plain(args: Result<BindingArgs, InvalidReason>, bind_ty: BindingType) -> Self {
        ...
        let any = match args {
            ...
            Ok(BindingArgs { path, visibility }) => {
                Any::binding(path, visibility, <T as GpuStore>::store_ty().expect("non-ZST type has store_ty"), bind_ty, as_ref)
            }
        };
...
impl<T: BufferFields + NoAtomics + NoBools, AS: BufferAddressSpace> BufferInner<T, AS> {
    ...
    pub fn new_fields(args: Result<BindingArgs, InvalidReason>, bind_ty: BindingType) -> Self {
        ...
        let block = T::get_bufferblock_type().expect("non-ZST type has bufferblock_type");
...
impl<T: GpuType + GpuStore + GpuSized, AS: BufferAddressSpace, L: ArrayLen> BufferInner<Array<T, L>, AS> {
    ...
    pub(crate) fn new_array(args: Result<BindingArgs, InvalidReason>, bind_ty: BindingType) -> Self {
        ...
        let store_ty = <Array<T, L> as GpuStore>::store_ty().expect("non-ZST type has store_ty");
        match L::LEN {
            // GpuSized
            Some(_) => {
                let as_ref = false;
...
impl<T: GpuStore + NoBools, AS: BufferAddressSpace, AM: AccessModeReadable> BufferRefInner<T, AS, AM> {
    ...
    {
        let as_ref = true;
        let store_ty = <T as GpuStore>::store_ty().expect("non-ZST type has store_ty");
        let any = match args {
            Err(reason) => Any::new_invalid(reason),
            Ok(BindingArgs { path, visibility }) => Any::binding(path, visibility, store_ty, bind_ty, as_ref),
        };
        ...
        T: BufferFields,
    {
        ...
        let block_any_ref = match args {
            ...
            Ok(BindingArgs { path, visibility }) => Any::binding(
                path,
                visibility,
                ir::StoreType::BufferBlock(T::get_bufferblock_type().expect("non-ZST type has bufferblock_type")),
                bind_ty,
                as_ref,
            ),
        };
    ...
    T: GpuSized
{
    ...
    fn store_ty() -> Option<ir::StoreType> {
        store_type_from_impl_category(T::impl_category())
    }
}

fn store_type_from_impl_category(category: GpuStoreImplCategory) -> Option<ir::StoreType> {
    match category {
        GpuStoreImplCategory::GpuType(ty) => ty,
        GpuStoreImplCategory::Fields(buffer_block) => {
            let buffer_block = buffer_block?;
            match buffer_block.clone().try_into() {
                Ok(sized) => Some(ir::StoreType::Sized(ir::SizedType::Structure(sized))),
                Err(_) => Some(ir::StoreType::BufferBlock(buffer_block)),
            }
        },
    }
}
    ...
    T: GpuType + GpuSized
{
    ...
    fn store_ty() -> Option<ir::StoreType> {
        T::sized_ty().map(ir::StoreType::RuntimeSizedArray)
    }
}
    ...
    AS: BufferAddressSpace,
{
    ...
    fn deref(&self) -> &T {
        ...
        match &self.inner {
            ...
            B::Zst(_) => unreachable!("ZST buffer has no GPU representation"),
        }
    }
}
                ...
                AM: AccessModeReadable
            {
                ...
                fn deref(&self) -> &Self::Target {
                    ...
                    match &self.inner {
                        ...
                        B::Plain(ref_t) => &ref_t,
                        B::Zst(_) => unreachable!("ZST buffer ref: GpuType types are not ZST"),
                    }
                }
            }
        )*
    ...
    AM: AccessModeReadable,
{
    ...
    fn deref(&self) -> &Self::Target {
        ...
        match &self.inner {
            ...
            B::Zst(_) => unreachable!("ZST buffer ref has no fields"),
        }
    }
}
