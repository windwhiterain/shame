...
pub trait Binding {
    ...
    /// shader type of `Self` (None for ZST bindings)
    ...
    SPP: Spp,
{
    ...
    fn store_ty() -> Option<ir::StoreType> {
        ...
        Some(ir::StoreType::Handle(ir::HandleType::SampledTexture(
            shape,
    ...
    fn new_binding(args: Result<BindingArgs, InvalidReason>) -> Self {
        let any = match args {
            ...
            Ok(BindingArgs { path, visibility }) => {
                Any::binding(path, visibility, Self::store_ty().unwrap(), Self::binding_type(), false)
            }
        };
    ...
    Binding for StorageTexture<Format, Coords, Access>
{
    ...
    fn store_ty() -> Option<ir::StoreType> {
        ...
        Some(ir::StoreType::Handle(ir::HandleType::StorageTexture(shape, format, access)))
    }
...
impl<M: SamplingMethod> Binding for Sampler<M> {
    ...
    fn store_ty() -> Option<ir::StoreType> { Some(ir::StoreType::Handle(ir::HandleType::Sampler(M::SAMPLING_METHOD))) }
