...
impl<T: GpuType + GpuStore + GpuSized, N: ArrayLen> GpuType for Array<T, N> {
    fn ty() -> ir::Type { ir::Type::Store(Self::store_ty().expect("Array is non-ZST")) }
...
impl<T: GpuType + GpuSized + GpuStore, N: ArrayLen> GpuStore for Array<T, N> {
    ...
    fn store_ty() -> Option<ir::StoreType> { Self::array_store_ty() }
...
impl<T: GpuType + GpuSized, N: ArrayLen> GpuAligned for Array<T, N> {
    fn aligned_ty() -> Option<ir::AlignedType> {
        let elem = T::sized_ty()?;
        match N::LEN {
            Some(len) => Some(ir::AlignedType::Sized(ir::SizedType::Array(Rc::new(elem), len))),
            None => Some(ir::AlignedType::RuntimeSizedArray(elem)),
        }
    }
}
...
impl<T: GpuType + GpuSized + GpuLayout, N: ArrayLen> GpuLayout for Array<T, N> {
    fn gpu_layout() -> TypeLayout {
        TypeLayout::from_array(TypeLayoutRules::Wgsl, &T::sized_ty().expect("Array element is non-ZST"), N::LEN)
    }
...
impl<T: GpuType + GpuSized, N: ArrayLen> Array<T, N> {
    fn array_store_ty() -> Option<ir::StoreType> {
        let element_ty = <T as GpuSized>::sized_ty()?;
        match N::LEN {
            Some(n) => Some(ir::StoreType::Sized(ir::SizedType::Array(Rc::new(element_ty), n))),
            None => Some(ir::StoreType::RuntimeSizedArray(element_ty)),
        }
    }
}
...
impl<T: GpuType + GpuSized, N: ArrayLen> From<Any> for Array<T, N> {
    fn from(any: Any) -> Self {
        super::typecheck_downcast(any, ir::Type::Store(Self::array_store_ty().expect("Array is non-ZST")), |any| Self {
            any,
            phantom: PhantomData,
        })
    }
    ...
    T::Gpu: GpuStore + GpuSized,
{
    ...
    fn to_gpu(&self) -> Self::Gpu {
        ...
        Any::new_array(Rc::new(T::Gpu::sized_ty().expect("Array element is non-ZST")), &anys).into()
    }
...
impl<T: GpuType + GpuStore + GpuSized + NoAtomics, const N: usize> Array<T, Size<N>> {
    ...
    #[track_caller]
    pub fn zero() -> Self { Any::new_default(Self::sized_ty().expect("Array is non-ZST")).into() }
