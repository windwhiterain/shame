...
///
pub trait GpuLayout {
    /// Whether this type is a Zero-Sized Type (ZST) on the GPU.
    ///
    /// ZSTs carry no data and need no IR generation. Types like `()` and
    /// empty `#[derive(GpuLayout)]` structs set this to `true`.
    #[doc(hidden)]
...
{
    fn get_sizedstruct_type() -> Option<ir::SizedStruct> {
        let struct_ = ir::SizedStruct::new_nonempty(
            std::stringify!(GpuType).into(),
            vec![
                ir::SizedField::new("a".into(), None, None, <vec<f32, x1> as GpuSized>::sized_ty().expect("non-ZST")),
                ir::SizedField::new("b".into(), None, None, <vec<u32, x1> as GpuSized>::sized_ty().expect("non-ZST")),
            ],
            ir::SizedField::new("c".into(), None, None, <vec<i32, x1> as GpuSized>::sized_ty().expect("non-ZST")),
        );
        match struct_ {
            Ok(s) => Some(s),
...
impl BufferFields for GpuT {
    ...
    fn get_bufferblock_type() -> Option<ir::BufferBlock> {
        ...
        let c = (
            std::stringify!(b).into(),
            <vec<i32, x1> as GpuAligned>::aligned_ty().expect("non-ZST"),
            None,
            None,
        );
...
impl<T: SizedFields + NoAtomics> ToGpuType for T {
    ...
    fn to_gpu(&self) -> Self::Gpu {
        Struct::<T>::from(Any::new_struct(T::get_sizedstruct_type().expect("non-ZST"), self.as_anys().borrow()))
    }
...
impl GpuAligned for GpuT {
    fn aligned_ty() -> Option<ir::AlignedType>
    where
...
impl GpuSized for GpuT {
    fn sized_ty() -> Option<ir::SizedType>
    where
        Self: for<'trivial_bound> GpuType,
    {
        unreachable!("Self: !GpuType")
...
impl CpuLayout for f64 {
    fn cpu_layout() -> TypeLayout {
        TypeLayout::from_rust_sized::<f64>(TypeLayoutSemantics::Vector(ir::Len::X1, Self::SCALAR_TYPE))
    }
    // fn gpu_type_layout() -> Option<Result<TypeLayout, ArrayElementsUnsizedError>> {
    //     Some(Ok(vec::<Self, x1>::gpu_layout()))
    // }
...
impl CpuLayout for u32 {
    fn cpu_layout() -> TypeLayout {
        TypeLayout::from_rust_sized::<u32>(TypeLayoutSemantics::Vector(ir::Len::X1, Self::SCALAR_TYPE))
    }
    // fn gpu_type_layout() -> Option<Result<TypeLayout, ArrayElementsUnsizedError>> {
    //     Some(Ok(vec::<Self, x1>::gpu_layout()))
    // }
...
impl CpuLayout for i32 {
    fn cpu_layout() -> TypeLayout {
        TypeLayout::from_rust_sized::<i32>(TypeLayoutSemantics::Vector(ir::Len::X1, Self::SCALAR_TYPE))
    }
    // fn gpu_type_layout() -> Option<Result<TypeLayout, ArrayElementsUnsizedError>> {
    //     Some(Ok(vec::<Self, x1>::gpu_layout()))
    // }
