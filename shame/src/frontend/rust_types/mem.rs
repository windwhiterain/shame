...
{
    Any::alloc(T::Gpu::sized_ty().expect("non-ZST"), init.to_any()).into()
}
...
pub fn workgroup_local<T: GpuType + GpuStore + GpuSized>() -> Ref<T, mem::WorkGroup> {
    ...
    Any::alloc_default_in(ir::AddressSpace::WorkGroup, T::sized_ty().expect("non-ZST")).into()
}
...
where
    T::Gpu: GpuStore + GpuSized,
{
    Any::alloc_explicit(ir::AddressSpace::Thread, T::Gpu::sized_ty().expect("non-ZST"), Some(init.to_any())).into()
}
