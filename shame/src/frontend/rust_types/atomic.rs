...
impl<T: ScalarTypeInteger> GpuType for Atomic<T> {
    fn ty() -> ir::Type { ir::Type::Store(Self::store_ty().expect("Atomic is non-ZST")) }
...
impl<T: ScalarTypeInteger> GpuSized for Atomic<T> {
    fn sized_ty() -> Option<ir::SizedType> { Some(ir::SizedType::Atomic(T::SCALAR_TYPE_INTEGER)) }
}
...
impl<T: ScalarTypeInteger> GpuStore for Atomic<T> {
    ...
    fn store_ty() -> Option<ir::StoreType> {
        <Self as GpuSized>::sized_ty().map(ir::StoreType::Sized)
    }
...
impl<T: ScalarTypeInteger> From<Any> for Atomic<T> {
    fn from(any: Any) -> Self {
        super::typecheck_downcast(any, <Self as GpuSized>::sized_ty().expect("Atomic is non-ZST").into(), |any| Self {
            any,
            phantom: PhantomData,
        })
    }
...
impl<T: ScalarTypeInteger> GpuLayout for Atomic<T> {
    fn gpu_layout() -> TypeLayout {
        TypeLayout::from_sized_ty(TypeLayoutRules::Wgsl, &<Self as GpuSized>::sized_ty().expect("Atomic is non-ZST"))
    }
