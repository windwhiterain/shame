...
impl<T: PackedScalarType, L: LenEven> PackedVec<T, L> {
    ...
    fn sized_ty_equivalent() -> SizedType {
        ...
        match get_type_description::<L, T>().byte_size() {
            Size::_2 => <vec<f16, x1> as GpuSized>::sized_ty().expect("vec is non-ZST"),
            Size::_4 => <vec<u32, x1> as GpuSized>::sized_ty().expect("vec is non-ZST"),
            Size::_8 => <vec<u32, x2> as GpuSized>::sized_ty().expect("vec is non-ZST"),
        }
    }
}
...
impl<T: PackedScalarType, L: LenEven> GpuAligned for PackedVec<T, L> {
    ...
    fn aligned_ty() -> Option<ir::AlignedType> {
        Some(ir::AlignedType::Sized(Self::sized_ty_equivalent()))
    }
}
