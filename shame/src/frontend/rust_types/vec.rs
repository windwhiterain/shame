...
impl<T: ScalarType, L: Len> GpuSized for vec<T, L> {
    fn sized_ty() -> Option<ir::SizedType> { Some(ir::SizedType::Vector(L::LEN, T::SCALAR_TYPE)) }
}
...
impl<T: ScalarType, L: Len> GpuLayout for vec<T, L> {
    fn gpu_layout() -> TypeLayout {
        TypeLayout::from_sized_ty(TypeLayoutRules::Wgsl, &<Self as GpuSized>::sized_ty().expect("vec is non-ZST"))
    }
...
impl<T: ScalarType, L: Len> GpuType for vec<T, L> {
    fn ty() -> ir::Type { ir::Type::Store(Self::store_ty().expect("vec is non-ZST")) }
