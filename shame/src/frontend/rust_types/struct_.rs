...
impl<T: SizedFields + GpuStore> GpuStore for Struct<T> {
    ...
    fn store_ty() -> Option<ir::StoreType> {
        <Self as GpuSized>::sized_ty().map(|s| ir::StoreType::Sized(s))
    }
...
impl<T: SizedFields + GpuStore> GpuAligned for Struct<T> {
    fn aligned_ty() -> Option<ir::AlignedType> {
        <Self as GpuSized>::sized_ty().map(ir::AlignedType::Sized)
    }
}

impl<T: SizedFields + GpuStore> GpuSized for Struct<T> {
    fn sized_ty() -> Option<ir::SizedType> {
        T::get_sizedstruct_type().map(ir::SizedType::Structure)
    }
}
...
where
    Struct<T>: Sized,
{
    fn ty() -> ir::Type { <Self as GpuSized>::sized_ty().map(ir::Type::from).unwrap_or(ir::Type::Unit) }

    #[track_caller]
    fn from_any_unchecked(any: Any) -> Self {
        let t = match T::get_sizedstruct_type() {
            None => T::from_anys(std::iter::empty()),
            Some(ty) => {
...
impl<T: SizedFields + GpuStore> From<Any> for Struct<T> {
    ...
    fn from(any: Any) -> Self {
        ...
        typecheck_downcast(
            any,
            <Self as GpuStore>::store_ty().map(ir::Type::Store).unwrap_or(ir::Type::Unit),
