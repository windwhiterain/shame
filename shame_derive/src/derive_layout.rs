...
pub fn impl_for_struct(
    ...
    let shame_crate_attr = util::find_literal_list_attr::<syn::Path>("shame_crate", &input.attrs)?;
    let re: TokenStream2 = match shame_crate_attr {
        Some((_, path)) => quote!(#path::__private::proc_macro_reexports),
        None => quote!(::shame::__private::proc_macro_reexports),
    };
    ...
    // ---- Zero-Sized Type (empty struct) special path ----
    if num_fields == 0 {
        match which_derive {
            WhichDerive::GpuLayout => {
                let zst_impl = quote! {
                    ...
                    where #where_clause_predicates
                    {
                        ...
                        fn gpu_layout() -> #re::TypeLayout {
                            #re::TypeLayout::new(Some(0), 1, #re::TypeLayoutSemantics::Structure(
                                std::rc::Rc::new(#re::StructLayout {
                                    name: std::stringify!(#derive_struct_ident).into(),
                                    fields: vec![],
                                })
                            ))
                        }
                        ...
                        fn cpu_type_name_and_layout() -> Option<Result<(std::borrow::Cow<'static, str>, #re::TypeLayout), #re::ArrayElementsUnsizedError>> {
                            #(#none_if_no_cpu_equivalent_type)*
                        }
                    }

                    impl<#generics_decl> #re::NoAtomics for #derive_struct_ident<#(#idents_of_generics),*> where #where_clause_predicates {}
                    impl<#generics_decl> #re::NoBools for #derive_struct_ident<#(#idents_of_generics),*> where #where_clause_predicates {}
                    impl<#generics_decl> #re::NoHandles for #derive_struct_ident<#(#idents_of_generics),*> where #where_clause_predicates {}
                    ...
                    where #where_clause_predicates {
                        ...
                        fn from_anys(mut anys: impl Iterator<Item = #re::Any>) -> Self {
                            #re::push_wrong_amount_of_args_error(anys.count(), 0, #re::call_info!());
                            Self {}
                        }
                    }
                    ...
                    where #where_clause_predicates {
                        fn sized_ty() -> Option<#re::ir::SizedType> where #triv Self: #re::GpuType { None }
                    }

                    impl<#generics_decl> #re::GpuAligned for #derive_struct_ident<#(#idents_of_generics),*>
                    where #where_clause_predicates {
                        fn aligned_ty() -> Option<#re::ir::AlignedType> where #triv Self: #re::GpuType { None }
                    }

                    impl<#generics_decl> #re::BufferFields for #derive_struct_ident<#(#idents_of_generics),*>
                    where #where_clause_predicates
                    {
                        fn as_anys(&self) -> impl std::borrow::Borrow<[#re::Any]> { [] as [#re::Any; 0] }
                        fn clone_fields(&self) -> Self { Self {} }
                        fn get_bufferblock_type() -> Option<#re::ir::BufferBlock> { None }
                    }

                    impl<#generics_decl> #re::GpuStore for #derive_struct_ident<#(#idents_of_generics),*>
                    where #where_clause_predicates
                    {
                        type RefFields<AS: #re::AddressSpace, AM: #re::AccessMode> = #re::EmptyRefFields;

                        fn store_ty() -> Option<#re::ir::StoreType> where #triv Self: #re::GpuType { None }

                        fn instantiate_buffer_inner<AS2: #re::BufferAddressSpace>(
                            _args: Result<#re::BindingArgs, #re::InvalidReason>,
                            _bind_ty: #re::BindingType
                        ) -> #re::BufferInner<Self, AS2>
                        where #triv Self: #re::NoAtomics + #re::NoBools
                        { unreachable!("ZST buffer guarded by IS_ZST") }

                        fn instantiate_buffer_ref_inner<AS2: #re::BufferAddressSpace, AM2: #re::AccessModeReadable>(
                            _args: Result<#re::BindingArgs, #re::InvalidReason>,
                            _bind_ty: #re::BindingType
                        ) -> #re::BufferRefInner<Self, AS2, AM2>
                        where #triv Self: #re::NoBools
                        { unreachable!("ZST buffer ref guarded by IS_ZST") }

                        fn impl_category() -> #re::GpuStoreImplCategory { #re::GpuStoreImplCategory::Fields(None) }
                    }

                    impl<#generics_decl> #re::SizedFields for #derive_struct_ident<#(#idents_of_generics),*>
                    where #where_clause_predicates
                    {
                        fn get_sizedstruct_type() -> Option<#re::ir::SizedStruct> { None }
                    }

                    impl<#generics_decl> #re::GetAllFields for #derive_struct_ident<#(#idents_of_generics),*>
                    where #where_clause_predicates
                    {
                        fn fields_as_anys_unchecked(_self_: #re::Any) -> impl std::borrow::Borrow<[#re::Any]> {
                            [] as [#re::Any; 0]
                        }
                    }
                    ...
                    where #where_clause_predicates
                    {
                        type Gpu = #re::Struct<Self>;
                        #[track_caller]
                        fn to_gpu(&self) -> Self::Gpu {
                            #re::Struct::from(#re::Any::new_invalid(#re::InvalidReason::CreatedWithNoActiveEncoding))
                        }
            ...
            WhichDerive::CpuLayout => {
                // ZST CpuLayout: size 0, alignment 1
                let zst_cpu = quote! {
        ...
        WhichDerive::GpuLayout => {
            ...
            let impl_fake_auto_traits = quote! {
                ...
                where
                    ...
                    #where_clause_predicates
                {
                    fn sized_ty() -> Option<#re::ir::SizedType> where
                ...
                where
                    ...
                    #where_clause_predicates
                {
                    fn aligned_ty() -> Option<#re::ir::AlignedType> where
                    #triv Self: #re::GpuType {
                        unreachable!("Self: !GpuType")
                    }
                }
            ...
            the `std::ops::Deref` target of `shame::Ref<{derive_struct_ident}>`"#
                ...
                Ok(quote! {
                        ...
                        #where_clause_predicates
                    {
                        ...
                        fn get_bufferblock_type() -> Option<#re::ir::BufferBlock> {
                            let mut fields = std::vec::Vec::from([
                                #(
                                    #re::ir::SizedField {
                                        ...
                                        ty: <#first_fields_type as #re::GpuSized>::sized_ty().expect("non-ZST"),
                                    }
                                ),*
                            ]);
                            ...
                            #[allow(clippy::no_effect)]
                            {
                                ...
                                match <#last_field_type as #re::GpuAligned>::aligned_ty().expect("non-ZST") {
                        ...
                        #where_clause_predicates
                    {
                        ...
                        fn store_ty() -> Option<#re::ir::StoreType> where
                        #triv Self: #re::GpuType {
                            unreachable!("Self: !GpuType")
                        }
                    ...
                    where
                        ...
                        #where_clause_predicates
                    {
                        fn get_sizedstruct_type() -> Option<#re::ir::SizedStruct> {
                            let struct_ = #re::ir::SizedStruct::new_nonempty(
                                ...
                                #re::ir::SizedField {
                                    ...
                                    custom_min_size: #last_field_size,
                                    custom_min_align: #last_field_align.map(|align: u32| TryFrom::try_from(align).expect("power of two validated during codegen")),
                                    ty: <#last_field_type as #re::GpuSized>::sized_ty().expect("non-ZST"),
                                }
                            );
                            match struct_ {
                                Ok(s) => Some(s),
